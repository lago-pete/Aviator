...
void readBme(BmeData &data)
{
  ...
  int32_t pVar1b = ((int32_t)bmeCalibPressure.par_p9 *
                    (int32_t)(((pressComp >> 3) * (pressComp >> 3)) >> 13)) >>
                   12;
...
// ---------------------------------------------------------------------------
// GPS Coords (Hour/Min/Sec) (lat/lon)
// ---------------------------------------------------------------------------
...
void displayGPS(String s)
{
  ...
  gpsReading.time.hour = tempTime.substring(0, 2).toInt();
  gpsReading.time.minute = tempTime.substring(2, 4).toInt();
  gpsReading.time.sec = tempTime.substring(4).toFloat();

  indexS = indexE + 1;
  indexE = s.indexOf(',', indexS);
  gpsReading.lat = s.substring(indexS, indexE).toDouble();

  indexS = indexE + 1;
  indexE = s.indexOf(',', indexS);
  gpsReading.latDir = s.substring(indexS, indexE).charAt(0);

  indexS = indexE + 1;
  indexE = s.indexOf(',', indexS);
  gpsReading.lon = s.substring(indexS, indexE).toDouble();

  indexS = indexE + 1;
  indexE = s.indexOf(',', indexS);
  gpsReading.lonDir = s.substring(indexS, indexE).charAt(0);

  indexS = indexE + 1;
  indexE = s.indexOf(',', indexS);
  gpsReading.fixQual = s.substring(indexS, indexE).toInt();

  indexS = indexE + 1;
  indexE = s.indexOf(',', indexS);
  gpsReading.satCount = s.substring(indexS, indexE).toInt();

  indexS = indexE + 1;
  indexE = s.indexOf(',', indexS);
  gpsReading.hdop = s.substring(indexS, indexE).toFloat();

  indexS = indexE + 1;
  indexE = s.indexOf(',', indexS);
  gpsReading.altitude = s.substring(indexS, indexE).toFloat();

  Serial.printf("  Time:     %02d:%02d:%05.2f UTC\n", gpsReading.time.hour, gpsReading.time.minute, gpsReading.time.sec);
...
// ---------------------------------------------------------------------------
bool checkI2CDevice(uint8_t addr, const char *name) // this was ai
{
...
void setup()
{
  ...
  for (int i = 0; i < n; i++)
  {
    Serial.printf("%s  RSSI:%d  Channel:%d  Encryption:%d\n",
                  WiFi.SSID(i).c_str(),
                  WiFi.RSSI(i),
                  WiFi.channel(i),
  ...
  WiFi.begin("GroundStationAP", "peter123");

  Serial.print("MAC Check for IP LOCK: ");
...
void loop()
{
  ...
  if (now - lastPrint > 500) // instead of Delay of (500) I going with this strat. This will allow the gps to continue cycling the buffer rather then having to wait for the delay to finish.
  {
    lastPrint = now;
    printDashboard(now);
    sendData.sendBme = bmeData;
    sendData.sendComp = compassData;
    sendData.sendGps = gpsReading;
    sendData.sendMpu = mpuData;

    udp.beginPacket(piIP, port);
    udp.write((const uint8_t *)&sendData, sizeof(sendData));
    udp.endPacket();
  }
}
